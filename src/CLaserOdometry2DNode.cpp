...
CLaserOdometry2DNode::CLaserOdometry2DNode() : CLaserOdometry2D() {
  ...
  laser_sub = n.subscribe<sensor_msgs::LaserScan>(
      laser_scan_topic, 1, &CLaserOdometry2DNode::LaserCallBack, this);

  // init pose??
  if (init_pose_from_topic != "") {
    initPose_sub = n.subscribe<nav_msgs::Odometry>(
        init_pose_from_topic, 1, &CLaserOdometry2DNode::initPoseCallBack, this);
...
bool CLaserOdometry2DNode::setLaserPoseFromTf() {
  ...
  // This allow estimation of the odometry with respect to the robot base
  // reference system.
...
bool CLaserOdometry2DNode::scan_available() { return new_scan_available; }
...
void CLaserOdometry2DNode::process(const ros::TimerEvent &) {
  if (is_initialized() && scan_available()) {
    ...
    publish();
    new_scan_available =
        false; // avoids the possibility to run twice on the same laser scan
  } else {
...
void CLaserOdometry2DNode::LaserCallBack(
...
void CLaserOdometry2DNode::initPoseCallBack(
    const nav_msgs::Odometry::ConstPtr &new_initPose) {
...
void CLaserOdometry2DNode::publish() {
  ...
  if (publish_tf) {
    ...
    odom_trans.transform.rotation =
        tf::createQuaternionMsgFromYaw(rf2o::getYaw(robot_pose_.rotation()));
    // send the transform
...
int main(int argc, char **argv) {
  ...
  timer_opt.callback =
      boost::bind(&rf2o::CLaserOdometry2DNode::process, &myLaserOdomNode, _1);
