/** ****************************************************************************************
 *  This node presents a fast and precise method to estimate the planar motion
 *of a lidar from consecutive range scans. It is very useful for the estimation
 *of the robot odometry from 2D laser range measurements. This module is
 *developed for mobile robots with innacurate or inexistent built-in odometry.
...
CLaserOdometry2D::CLaserOdometry2D()
    : verbose(false), module_initialized(false), first_laser_scan(true),
      last_increment_(Pose3d::Identity()),
      laser_pose_on_robot_(Pose3d::Identity()),
      laser_pose_on_robot_inv_(Pose3d::Identity()),
      laser_pose_(Pose3d::Identity()), laser_oldpose_(Pose3d::Identity()),
      robot_pose_(Pose3d::Identity()), robot_oldpose_(Pose3d::Identity()) {
  //
}
...
bool CLaserOdometry2D::is_initialized() { return module_initialized; }
...
void CLaserOdometry2D::init(const sensor_msgs::LaserScan &scan,
  ...
  robot_initial_pose = Eigen::Quaterniond(
      initial_robot_pose.orientation.w, initial_robot_pose.orientation.x,
      initial_robot_pose.orientation.y, initial_robot_pose.orientation.z);
  ...
  const unsigned int pyr_levels =
      std::round(std::log2(round(float(width) / float(cols)))) + ctf_levels;
...
CLaserOdometry2D::getIncrementCovariance() const {
  return cov_odo;
}

CLaserOdometry2D::Pose3d &CLaserOdometry2D::getPose() { return robot_pose_; }
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan &scan) {
  ...
  for (unsigned int i = 0; i < ctf_levels; i++) {
    ...
    image_level =
        ctf_levels - i +
        std::round(std::log2(std::round(float(width) / float(cols)))) - 1;
...
void CLaserOdometry2D::createImagePyramid() {
  ...
  // used in the odometry computation (because we sometimes want to finish with
  // lower resolutions)

  unsigned int pyr_levels =
      std::round(std::log2(std::round(float(width) / float(cols)))) +
      ctf_levels;
...
void CLaserOdometry2D::calculateCoord() {
  for (unsigned int u = 0; u < cols_i; u++) {
    if ((range_old[image_level](u) == 0.f) ||
      ...
      range_inter[image_level](u) =
          0.5f * (range_old[image_level](u) + range_warped[image_level](u));
      xx_inter[image_level](u) =
          0.5f * (xx_old[image_level](u) + xx_warped[image_level](u));
      yy_inter[image_level](u) =
          0.5f * (yy_old[image_level](u) + yy_warped[image_level](u));
    }
  }
}
...
void CLaserOdometry2D::calculaterangeDerivativesSurface() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++)
    dtita(u) =
        (rtita(u - 1) *
             (range_inter[image_level](u + 1) - range_inter[image_level](u)) +
         rtita(u) *
             (range_inter[image_level](u) - range_inter[image_level](u - 1))) /
        (rtita(u) + rtita(u - 1));
  ...
  // for (unsigned int u=1; u<cols_i-1; u++)
  //{
  //	svector.at(0) = dtita(u-1); svector.at(1) = dtita(u); svector.at(2) =
  //dtita(u+1); 	std::sort(svector.begin(), svector.end()); 	dtitamed(u) =
  //svector.at(1);
...
void CLaserOdometry2D::computeNormals() {
  ...
  for (unsigned int u = 0; u < cols_i; u++) {
    if (null(u) == 0.f) {
      ...
      const float alfa =
          -std::atan2(2.f * dtita(u), 2.f * range[image_level](u) * incr_tita);
...
void CLaserOdometry2D::computeWeights() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++)
    if (null(u) == 0) {
      ...
      //derivatives
      //-----------------------------------------------------------------------
      const float ini_dtita =
          range_old[image_level](u + 1) - range_old[image_level](u - 1);
      const float final_dtita =
          range_warped[image_level](u + 1) - range_warped[image_level](u - 1);
...
void CLaserOdometry2D::solveSystemOneLevel() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++)
    if (null(u) == 0) {
      ...
      // Fill the matrix A
      A(cont, 0) = tw * (std::cos(tita) + dtita(u) * kdtita * std::sin(tita) /
                                              range_inter[image_level](u));
      A(cont, 1) = tw * (std::sin(tita) - dtita(u) * kdtita * std::cos(tita) /
                                              range_inter[image_level](u));
      A(cont, 2) =
          tw * (-yy[image_level](u) * std::cos(tita) +
                xx[image_level](u) * std::sin(tita) - dtita(u) * kdtita);
      B(cont, 0) = tw * (-dt(u));
  ...
  res = A * Var - B;
  cov_odo =
      (1.f / float(num_valid_range - 3)) * AtA.inverse() * res.squaredNorm();
    ...
    for (unsigned int u = 1; u < cols_i - 1; u++)
      if (null(u) == 0) {
        const float res_weight =
            std::sqrt(1.f / (1.f + ((k * res(cont)) * (k * res(cont)))));
...
void CLaserOdometry2D::Reset(
    const Pose3d &ini_pose /*, CObservation2DRangeScan scan*/) {
  // Set the initial pose
...
void CLaserOdometry2D::performWarping() {
  ...
  for (unsigned int j = 0; j < cols_i; j++) {
    if (range[image_level](j) > 0.f) {
      ...
      const float x_w = acu_trans(0, 0) * xx[image_level](j) +
                        acu_trans(0, 1) * yy[image_level](j) + acu_trans(0, 2);
      const float y_w = acu_trans(1, 0) * xx[image_level](j) +
                        acu_trans(1, 1) * yy[image_level](j) + acu_trans(1, 2);
...
bool CLaserOdometry2D::filterLevelSolution() {
  ...
  if (eigensolver.info() != Eigen::Success) {
    ROS_WARN_COND(verbose, "[rf2o] ERROR: Eigensolver couldn't find a "
                           "solution. Pose is not updated");
    return false;
  }

  // First, we have to describe both the new linear and angular speeds in the
  // "eigenvector" basis
  //-------------------------------------------------------------------------------------------------
  ...
  assert((kai_loc_level_).isApprox(Bii * kai_b, 1e-5) &&
         "Ax=b has no solution." && __LINE__);

  // Second, we have to describe both the old linear and angular speeds in the
  // "eigenvector" basis too
  //-------------------------------------------------------------------------------------------------
  ...
  if (acu_trans(0, 0) > 1.f)
    ...
    kai_loc_sub(2) =
        -fps * std::acos(acu_trans(0, 0)) * rf2o::sign(acu_trans(1, 0));
  }
  ...
  assert((kai_loc_sub).isApprox(Bii * kai_b_old, 1e-5) &&
         "Ax=b has no solution." && __LINE__);
  ...
  // Transform filtered speed to local reference frame and compute
  // transformation
  Eigen::Matrix<float, 3, 1> kai_loc_fil =
      Bii.inverse().colPivHouseholderQr().solve(kai_b_fil);

  assert((kai_b_fil).isApprox(Bii.inverse() * kai_loc_fil, 1e-5) &&
         "Ax=b has no solution." && __LINE__);
...
void CLaserOdometry2D::PoseUpdate() {
  ...
  ROS_INFO_COND(verbose, "[rf2o] LASERodom = [%f %f %f]",
                laser_pose_.translation()(0), laser_pose_.translation()(1),
                rf2o::getYaw(laser_pose_.rotation()));

  // Compose Transformations
  robot_pose_ = laser_pose_ * laser_pose_on_robot_inv_;

  ROS_INFO_COND(verbose, "BASEodom = [%f %f %f]", robot_pose_.translation()(0),
  ...
  // last_odom_time -> The time of the previous scan lasser used to estimate the
  // pose
  //-------------------------------------------------------------------------------------
  ...
  // double lin_speed = sqrt(
  // mrpt::math::square(robot_oldpose.x()-robot_pose.x()) +
  // mrpt::math::square(robot_oldpose.y()-robot_pose.y()) )/time_inc_sec;
  ...
  /*
    ...
    double sum = std::accumulate(last_m_lin_speeds.begin(),
    last_m_lin_speeds.end(), 0.0); lin_speed = sum / last_m_lin_speeds.size();

    last_m_ang_speeds.push_back(ang_speed);
    if (last_m_ang_speeds.size()>4)
        last_m_ang_speeds.erase(last_m_ang_speeds.begin());
    double sum2 = std::accumulate(last_m_ang_speeds.begin(),
    last_m_ang_speeds.end(), 0.0); ang_speed = sum2 / last_m_ang_speeds.size();
    */
}
